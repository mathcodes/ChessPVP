...
$(document).ready(function () {
  ...
  $(function () {
    ...
    for (var i = 0; i < 8; i++) {
      for (var j = 0; j < 8; j++) {
        var box = $('#box-' + i + '-' + j)
        // sets up odd vs even light vs dark boxes
        if ((i + j) % 2 !== 0) {
  ...
  var getNextMoves = function (selectedPiece, selectedBox) {
    ...
    switch (type) {
		// PAWN moves based on changes in i and j
      case 'pawn':
        if (color === 'black') {
          var moves = [
            [0, 1],
        ...
        break
      case 'rook':
		// ROOK moves based on direction * moves
        var moves = [
          [0, 1],
          [0, -1],
          [1, 0],
        ...
        break
      case 'knight':
		// KNIGHT moves based on possibilities
        var moves = [
          [-1, -2],
          [-2, -1],
          [1, -2],
      ...
      case 'bishop':
        ...
        break
      case 'queen':
		// moves 1st round like ROOK and 2nd like BISHOP (has the option of either)
        var moves1 = [
          [1, 1],
          [1, -1],
          [-1, 1],
        ...
        break
      case 'king':
		// can move any way in any direction, once
        var moves = [
          [1, 1],
          [1, -1],
          [-1, 1],
  ...
  var setNewBoard = function (box, i, j) {
	  // if j === 8th row
    if (j === 7) {
      //sets up pieces on 8th row
      if (i === 0 || i === 7) {
        // first and last column = ROOK
        setPiece(box, 'white', 'rook')
      } else if (i === 1 || i === 6) {
        // 2nd and 7th column = KNIGHT
        setPiece(box, 'white', 'knight')
      } else if (i === 2 || i === 5) {
        // 3rd and 6th column = BISHOP
        setPiece(box, 'white', 'bishop')
      } else if (i === 3) {
        // sets up QUEEN in 4th position
        setPiece(box, 'white', 'queen')
      } else if (i === 4) {
        // sets up KING in 3rd position
        setPiece(box, 'white', 'king')
      }
    } else if (j === 6) {
      // sets  up white pawns on 7th row
      setPiece(box, 'white', 'pawn')
    } else if (j === 1) {
      // sets up blacks pawns on 2nd row
	  setPiece(box, 'black', 'pawn')
	  // if j === 1st row, sets up rest of black pieces (same as white pieces above)
    } else if (j === 0) {
      if (i === 0 || i === 7) {
